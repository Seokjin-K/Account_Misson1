...
// Mocking된 빈들이 AccountController와 함께 테스트 컨테이너에 등록된다.
@WebMvcTest(AccountController.class)
class AccountControllerTest {

    // @WebMvcTest(AccountController.class)에 이미 존재하기 때문에 injection하지 않음
    // @MockBean이 자동으로 빈으로 등록되어 AccountController에 주입된다.
    @MockBean
    private AccountService accountService;

    // 주입된 어플리케이션에 MockMvc가 요청을 날려 테스트한다.
    @Autowired
    ...
    void successGetAccount() throws Exception {
        ...
        mockMvc.perform(get("/account/876"))
                .andDo(print()) // get일 때의 응답과 요청을 콘솔에 출력
